...
class Particle {
  constructor(x, y, brightness, r, g, b) {
    ...
    this.r = Math.min(255, r * 0.2 + 240 * 0.8);
    this.g = Math.min(255, g * 0.2 + 185 * 0.8);
    this.b = Math.min(255, b * 0.2 +  40 * 0.8);
    ...
    this.fx = 0.00004 + Math.random() * 0.00006;
    this.fy = 0.00003 + Math.random() * 0.00005;

    const edginess = 1 - Math.abs(brightness - 0.5) * 1.5;
    const e = Math.max(0, edginess);
    this.ax = 0.3 + e * 1.2 + Math.random() * 0.5;
    this.ay = 0.2 + e * 0.8 + Math.random() * 0.4;

    this.riseSpeed = 0.0008 + Math.random() * 0.0012;
